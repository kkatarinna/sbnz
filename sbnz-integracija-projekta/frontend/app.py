...
@app.route("/run-logs-data-exfiltration", methods=["POST"])
def run_logs_data_exfiltration():
    drools_url = "http://localhost:8080/dataExfiltrationLogs"
    try:
        response = requests.post(drools_url, json={})
        response.raise_for_status()
        result = response.json()
...
@app.route("/run-logs-sql-injection", methods=["POST"])
def run_logs_sql_injection():
    drools_url = "http://localhost:8080/sqlInjection"
    try:
        response = requests.post(drools_url, json={})
        response.raise_for_status()
        result = response.json()
